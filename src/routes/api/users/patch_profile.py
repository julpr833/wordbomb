...
def patch_profile(username):
    ...
    print(new_password, new_pass_confirm)
    ...
    with mysql.get_db().cursor() as cursor:
        ...
        if new_username:
            validator.is_valid_username(new_username) 
            validator.username_exists(new_username)
            updates["Username"] = new_username

        # Si actualiza contraseña
        if (new_password and new_pass_confirm):
            validator.is_strong_password(new_password)  
            validator.passwords_match(new_password, new_pass_confirm)
        ...
        if updates == {}:
            return {"error": "No hay cambios para realizar"}, 400
